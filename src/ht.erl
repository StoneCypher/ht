...
stub_serve(ConfigPL) when is_list(ConfigPL) -> 
    ...
    spawn( fun() -> setup_loop(Handler, Port, LSock) end ).





setup_loop(Handler, Port, LSock) ->

    ListenLoop = spawn_link(fun() -> listen_loop(Handler, Port, LSock) end),

    loop(ListenLoop, LSock).





listen_loop(Handler, Port, LSock) ->
